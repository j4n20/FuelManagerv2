using System.Collections.Generic;

namespace SupremeLollipopService
{
    public class FEmployeeRepo
    {
        public void Delete(int CustomerId)
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                using (var transaction = session.BeginTransaction())
                {
                    var returnList = session.QueryOver<FEmployee>()
                        .Where(t => t.Id == CustomerId).List()[0];
                    session.Delete(returnList);
                    transaction.Commit();
                }
            }
        }

        public List<FEmployee> GetAll()
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                var result = session.QueryOver<FEmployee>().List();
                return result as List<FEmployee>;
            }
        }

        public FEmployee GetFEmployee(int FEmployeeId)
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                var returnList = session.QueryOver<FEmployee>()
                    .Where(t => t.Id == FEmployeeId).List()[0];
                return returnList;
            }
        }

        public void SaveOrUpdate(FEmployee entity)
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                using (var transaction = session.BeginTransaction())
                {
                    session.SaveOrUpdate(entity);
                    transaction.Commit();
                }
            }
        }
    }
}